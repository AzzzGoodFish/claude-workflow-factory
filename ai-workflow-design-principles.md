...
| name | 节点名称 | ✅ |
| description | 节点描述 | 建议 |
| input.contract | 引用的契约名称 | ✅ |
| input.source | 输入文件路径模板 | ✅ |
| output.contract | 引用的契约名称 | ✅ |
| output.target | 输出文件路径模板 | ✅ |
| implementation | 节点实现 | ✅ |
...
| 变量名 | 类型 | 必须 | 说明 |
|-------|------|------|------|
| VAR_A | string | ✅ | 示例：某个执行所需的关键参数 |
| VAR_B | string | ✅ | 示例：源码/数据所在目录 |
| VAR_C | string | ✅ | 示例：输出目录 |

**layout（存储布局）**

| 字段 | 类型 | 必须 | 说明 | 示例（模板） |
|------|------|------|------|--------------|
| intermediate | string | ✅ | 中间产物目录 | `$OUTPUT/intermediate/{entity}/` |
| final | string | ✅ | 最终产物目录 | `$OUTPUT/{entity}/` |
| temp | string | ✅ | 临时目录 | `$OUTPUT/.temp/` |

**state（共享状态，运行时生成）**

| 键 | 类型 | 必须 | 说明 | populated_by |
|----|------|------|------|--------------|
| key_a | any | 可选 | 示例：某个阶段产出的共享数据 | `stage-x` |
